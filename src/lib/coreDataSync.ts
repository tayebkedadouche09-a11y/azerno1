...
function dependencyRank(operation: QueueOperation): number {
  const ranks: Record<string, number> = { customer:10, supplier:20, product:30, variant:40, order:50, expense:60, purchase:70, payment:80, return:90, waste:90, production:90, livestock_event:90, feed:90 };
  return ranks[operation.entity] ?? 100;
}

function pendingOperations(): QueueOperation[] {
  return offlineQueue.list().filter(operation => operation.status !== 'failed').sort((a,b) => dependencyRank(a)-dependencyRank(b) || a.createdAt.localeCompare(b.createdAt));
}
...
export async function pullRemoteChanges() {
  ...
  pullInFlight=(async()=>{ let cursor=getPullCursor(), changed=0; try {
    for(let page=0;page<20;page++) { const result=await api.syncPull(getDeviceId(),cursor); changed+=result.items.length; cursor=result.nextSince||cursor; setPullCursor(cursor); if(!result.items.length || result.items.length<500) break; }
    if(changed) { await dataRepository.clearCache(); await refreshCoreData(); if(typeof window!=='undefined') window.dispatchEvent(new CustomEvent(DATA_CHANGED_EVENT)); }
    return changed;
...
export function startCoreDataSync(onRefresh?:()=>void) {
  const handleOnline=async()=>{ await flushCoreCreateQueue(); await pullRemoteChanges(); await refreshCoreData(); onRefresh?.(); };

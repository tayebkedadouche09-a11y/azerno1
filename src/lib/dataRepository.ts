...
function readCache<T>(key: CacheKey): T[] { try { const raw = localStorage.getItem(cacheKey(key)); return raw ? JSON.parse(raw) as T[] : []; } catch { return []; } }
function writeCache<T>(key: CacheKey, items: T[]) { try { localStorage.setItem(cacheKey(key), JSON.stringify(items)); } catch {} }
...
async function read<T>(key: CacheKey, loader: () => Promise<{ items: RawRecord[] }>, normalize: (raw: RawRecord) => T[]): Promise<{ items: T[]; source: 'api' | 'cache' }> {
  if (navigator.onLine) { try { const result = await loader(); const normalized = normalize({ items: result.items }); writeCache(key, normalized); return { items: normalized, source: 'api' }; } catch (error) { if (!isOfflineError(error)) throw error; } }
...
async function createWithFallback<T>(key: CacheKey, entity: 'customer' | 'product' | 'order', payload: RawRecord, creator: () => Promise<RawRecord>, normalize: (raw: RawRecord) => T) {
  if (navigator.onLine) { try { const created = await creator(); const item = normalize(created); const items = readCache<T>(key); writeCache(key, [item, ...items.filter(existing => String((existing as any).id) !== String((item as any).id))]); return { item, source: 'api' as const, queued: false }; } catch (error) { if (!isOfflineError(error)) throw error; } }
  ...
  writeCache(key, [localItem, ...readCache<T>(key)]);
...
export const dataRepository = {
  ...
  productCategories: async () => { const result = await api.productCategories(); return result.items; },
  ...
  clearCache: () => (['customers', 'products', 'orders'] as CacheKey[]).forEach(key => localStorage.removeItem(cacheKey(key))),
};

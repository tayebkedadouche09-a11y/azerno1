    ...
    <div className="space-y-4 p-4 pb-24 max-w-2xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button
          ...
          onClick={retryAll}
          disabled={syncing}
          className="inline-flex items-center gap-2 rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 min-h-[44px]"
        >
        ...
        {items.map((item) => (
          ...
          >
            ...
            {(item.status === 'conflict' || item.status === 'rejected' || item.status === 'failed') && item.payload != null && (
              <pre className="mt-2 max-h-28 overflow-auto text-[10px] bg-slate-100 dark:bg-slate-800 p-2 rounded-lg text-left whitespace-pre-wrap">
            ...
            {(item.status === 'failed' || item.status === 'conflict' || item.status === 'rejected') && (
              <button
                type="button"
                className="mt-2 min-h-[40px] text-xs font-semibold text-teal-700"

...
router.post('/purchases', requireRole('owner','manager'), async(req,res,next)=>{
  try{
    ...
    const result=await transaction(async client=>{
      const idempotencyKey=req.body?.idempotencyKey?String(req.body.idempotencyKey):null;
      if(idempotencyKey){const existing=await client.query(`SELECT id FROM purchases WHERE idempotency_key=$1 LIMIT 1`,[idempotencyKey]);if(existing.rows[0]){const existingPurchase=await client.query(`SELECT * FROM purchases WHERE id=$1`,[existing.rows[0].id]);return existingPurchase.rows[0];}}

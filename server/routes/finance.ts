...
async function nextNumber(client: import('pg').PoolClient, key: string, prefix: string) {
  await client.query(`INSERT INTO app_settings(key,value) VALUES($1,$2) ON CONFLICT(key) DO NOTHING`, [key, JSON.stringify({ counter: 0 })]);
  ...
  await client.query(`UPDATE app_settings SET value=$2, updated_at=NOW() WHERE key=$1`, [key, JSON.stringify({ counter })]);
  return `${prefix}${String(counter).padStart(6, '0')}`;
}

router.get('/suppliers', async (_req, res, next) => {
  try {
    const result = await query(`SELECT s.*, COALESCE((SELECT SUM(p.total) FROM purchases p WHERE p.supplier_id=s.id AND p.status<>'cancelled'),0) AS total_purchases, COALESCE((SELECT SUM(p.paid_total) FROM purchases p WHERE p.supplier_id=s.id AND p.status<>'cancelled'),0) AS total_paid, GREATEST(COALESCE((SELECT SUM(p.total-p.paid_total) FROM purchases p WHERE p.supplier_id=s.id AND p.status<>'cancelled'),0),0) AS balance_owed FROM suppliers s WHERE s.active=TRUE ORDER BY s.name`);
...
router.post('/suppliers', requireRole('owner','manager'), async (req,res,next) => {
  try {
    ...
    res.status(201).json({...row,total_purchases:0,total_paid:0,balance_owed:0});
  } catch(e){ next(e); }
});
  ...
  try{
    ...
    const result=await transaction(async client=>{
      ...
      if(idempotencyKey){
        ...
        if(existing.rows[0]){ const existingPurchase=await client.query(`SELECT * FROM purchases WHERE id=$1`,[existing.rows[0].id]); return existingPurchase.rows[0]; }
      }
      let total=0;
      ...
      for(const item of normalized){
        const purchaseItem = await client.query<{id:string}>(`INSERT INTO purchase_items(purchase_id,variant_id,description,quantity,unit_cost,total) VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,[p.rows[0].id,item.variantId,item.description,item.quantity,item.unitCost,item.total]);
        const purchaseItemId = purchaseItem.rows[0].id;
        await client.query(`INSERT INTO inventory_balances(variant_id,quantity,reserved_quantity) VALUES($1,$2,0) ON CONFLICT(variant_id) DO UPDATE SET quantity=inventory_balances.quantity+$2,updated_at=NOW()`,[item.variantId,item.quantity]);
        await client.query(`INSERT INTO inventory_movements(variant_id,movement_type,quantity,reference_type,reference_id,idempotency_key,created_by) VALUES($1,'purchase',$2,'purchase',$3,$4,$5)`,[item.variantId,item.quantity,p.rows[0].id,`purchase:${p.rows[0].id}:item:${purchaseItemId}`,req.user!.id]);
      }
...
router.post('/supplier-payments', requireRole('owner','manager'), async(req,res,next)=>{
  try {
    const purchaseId=String(req.body?.purchaseId??''); const amount=Number(req.body?.amount);
    ...
    const result=await transaction(async client=>{
      ...
      const row=purchase.rows[0]; if(!row) throw new Error('Purchase not found');
      ...
      if(key){
        ...
        if(existing.rows[0]){ if(existing.rows[0].purchase_id !== purchaseId) throw new Error('Idempotency key is already used for another purchase'); return existing.rows[0]; }
      }

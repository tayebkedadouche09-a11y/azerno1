...
router.get('/categories', async (_req, res, next) => {
  try {
    const result = await query(`SELECT id, name, name_ar, active FROM product_categories WHERE active = TRUE ORDER BY name`);
...
router.patch('/variants/:id', requireRole('owner', 'manager'), async (req, res, next) => {
  try {
    const result = await transaction(async client => {
      ...
      const retailPrice = Number(req.body?.retailPrice ?? before.retail_price);
      const wholesalePrice = Number(req.body?.wholesalePrice ?? before.wholesale_price);
      const productionCost = Number(req.body?.cost ?? req.body?.productionCost ?? before.production_cost);
      const minStock = Number(req.body?.minStock ?? before.min_stock);
      const barcode = req.body?.barcode ?? before.barcode;
      ...
      const updated = await client.query(`
        ...
        SET name=$2, retail_price=$3, wholesale_price=$4, production_cost=$5, min_stock=$6, barcode=$7, updated_at=NOW()
        WHERE id=$1 RETURNING *
      `, [req.params.id, name, retailPrice, wholesalePrice, productionCost, minStock, barcode]);

      if (Number(before.retail_price) !== retailPrice || Number(before.wholesale_price) !== wholesalePrice || Number(before.production_cost) !== productionCost) {
        await client.query(`INSERT INTO price_history(variant_id,retail_price,wholesale_price,production_cost,changed_by) VALUES($1,$2,$3,$4,$5)`, [req.params.id, retailPrice, wholesalePrice, productionCost, req.user!.id]);
      }
...
router.post('/stock-adjustments', requireRole('owner', 'manager'), async (req, res, next) => {
  try {
    const result = await transaction(async client => {
      ...
      const balance = await client.query(`SELECT quantity,reserved_quantity FROM inventory_balances WHERE variant_id=$1 FOR UPDATE`, [variantId]);
      if (!balance.rows[0]) throw new Error('Inventory balance not found');
      const currentQuantity = Number(balance.rows[0].quantity);
      const reservedQuantity = Number(balance.rows[0].reserved_quantity);
      if (targetQuantity < reservedQuantity) throw new Error('Physical stock cannot be lower than reserved stock');
      const delta = targetQuantity - currentQuantity;
      if (delta === 0) return { variantId, quantity: targetQuantity, reservedQuantity, delta: 0 };

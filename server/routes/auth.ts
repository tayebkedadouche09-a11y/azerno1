...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const result = await query<{ id: string; name: string; role: 'owner' | 'manager' | 'worker'; password_hash: string }>(
    ...
    await query(
      ...
      [user.id, hashToken(token), Number(process.env.ACCESS_TOKEN_TTL_SECONDS ?? 60 * 60 * 12)],
    );
...
router.post('/bootstrap-owner', async (req, res, next) => {
  try {
    ...
    const result = await query<{ id: string; name: string; role: 'owner' }>(

...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const result = await query<{ id: string; name: string; password_hash: string }>(
      `SELECT id, name, password_hash FROM users WHERE active = TRUE AND (LOWER(email) = $1 OR phone = $1) LIMIT 1`,
      [identifier],
    );
    ...
    const memberships = await query<{ company_id: string; company_name: string; role: 'owner' | 'manager' | 'worker' }>(
      `SELECT cm.company_id, c.name AS company_name, cm.role
       ...
       WHERE cm.user_id = $1 AND c.active = TRUE ORDER BY c.created_at ASC`, [user.id]);
    ...
    await query(
      `INSERT INTO sessions(user_id, company_id, token_hash, expires_at) VALUES ($1, $2, $3, NOW() + ($4 * INTERVAL '1 second'))`,
...
router.post('/companies', requireAuth, requireRole('owner'), async (req, res, next) => {
  try {
    ...
    const rawSlug = String(req.body?.slug ?? name.toLowerCase().replace(/[^a-z0-9]+/g, '-')).trim().replace(/^-+|-+$/g, '');
    if (!name || !rawSlug) return res.status(400).json({ error: 'Company name and slug are required' });
    const result = await transaction(async (client) => {
      const company = await client.query(`INSERT INTO companies(name,slug) VALUES($1,$2) RETURNING id,name,slug,active`, [name, rawSlug]);
      await client.query(`INSERT INTO company_memberships(company_id,user_id,role) VALUES($1,$2,'owner')`, [company.rows[0].id, req.user!.id]);
...
router.post('/bootstrap-owner', async (req, res, next) => {
  try {
    ...
    const result = await transaction(async (client) => {
      const company = await client.query<{ id: string; name: string }>(`UPDATE companies SET name=$1, updated_at=NOW() WHERE slug='azrnou-default' RETURNING id, name`, [String(req.body?.companyName ?? 'AZRNOU Entreprise').trim() || 'AZRNOU Entreprise']);
      const created = await client.query<{ id: string; name: string; role: 'owner' }>(`INSERT INTO users(name, email, password_hash, role) VALUES ($1, $2, $3, 'owner') RETURNING id, name, role`, [name, email, passwordHash]);

...
router.post('/deliveries', requireRole('owner','manager','worker'), async (req, res, next) => {
  try {
    ...
    const created = await transaction(async client => {
      ...
      const delivery = await client.query(`INSERT INTO deliveries(number,order_id,delivery_date,status,delivery_person,address,notes,created_by) VALUES($1,$2,$3,'pending',$4,$5,$6,$7) RETURNING *`, [number, orderId, req.body?.deliveryDate ?? new Date().toISOString().slice(0,10), req.body?.deliveryPerson ?? null, req.body?.address ?? null, req.body?.notes ?? null, req.user!.id]);
      let allDelivered = true;
      for (const item of items) {
        const qty = Number(item.quantity);
        if (!item.variantId || !Number.isFinite(qty) || qty <= 0) throw new Error('Invalid delivery quantity');
        const oi = await client.query(`SELECT id, quantity, delivered_quantity FROM order_items WHERE order_id=$1 AND variant_id=$2 FOR UPDATE`, [orderId, item.variantId]);
...
router.get('/invoices', async (_req, res, next) => {
  try { const result = await query(`SELECT i.*, c.name AS customer_name, o.number AS order_number FROM invoices i LEFT JOIN customers c ON c.id=i.customer_id LEFT JOIN orders o ON o.id=i.order_id ORDER BY i.invoice_date DESC, i.created_at DESC`); res.json({ items: result.rows }); } catch(e) { next(e); }
});

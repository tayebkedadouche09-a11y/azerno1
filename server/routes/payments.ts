...
router.post('/payments', async (req, res, next) => {
  try {
    ...
    if (!orderId || !Number.isFinite(amount) || amount <= 0) {
      return res.status(400).json({ error: 'Order and positive payment amount are required' });
    }
    ...
    const payment = await transaction(async client => {
      ...
      const order = await client.query(
        'SELECT id,customer_id,total,paid_total FROM orders WHERE id=$1 FOR UPDATE',
        [orderId]
      );
      ...
      await client.query(
        `UPDATE orders
         SET paid_total=$2,
             payment_status=$3,
             status=CASE WHEN status='delivered' AND $2>=total THEN 'paid' ELSE status END,
             updated_at=NOW()
         WHERE id=$1`,
        [orderId, paidTotal, paymentStatus]
      );

      await client.query(
        `INSERT INTO audit_logs(user_id,action,entity_type,entity_id,after_data)
         VALUES($1,'create','payment',$2,$3)`,
        [req.user!.id, inserted.rows[0].id, JSON.stringify(inserted.rows[0])]
      );

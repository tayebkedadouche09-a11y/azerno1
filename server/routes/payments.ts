...
router.post('/payments', requireRole('owner','manager'), async (req, res, next) => {
  try {
    const orderId=String(req.body?.orderId??''); const amount=Number(req.body?.amount); const idempotencyKey=req.body?.idempotencyKey?String(req.body.idempotencyKey):null;
    ...
    const payment=await transaction(async client=>{
      if(idempotencyKey){const existing=await client.query('SELECT * FROM payments WHERE idempotency_key=$1 LIMIT 1',[idempotencyKey]);if(existing.rows[0])return existing.rows[0];}
      const order=await client.query('SELECT id,customer_id,total,paid_total,status FROM orders WHERE id=$1 FOR UPDATE',[orderId]); if(!order.rows[0])throw new Error('Order not found');
      if(String(order.rows[0].status)==='cancelled')throw new Error('Cannot record payment for a cancelled order');
      const remaining=Number(order.rows[0].total)-Number(order.rows[0].paid_total); if(amount>remaining)throw new Error('Payment exceeds outstanding balance');
      ...
      const paidTotal=Number(order.rows[0].paid_total)+amount; const paymentStatus=paidTotal>=Number(order.rows[0].total)?'paid':'partially_paid';

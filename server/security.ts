...
export function createAccessToken(userId: string, companyId: string) {
  const now = Math.floor(Date.now() / 1000);
  const header = base64url(JSON.stringify({ alg: 'HS256', typ: 'JWT' }));
  const payload = base64url(JSON.stringify({ sub: userId, companyId, iat: now, exp: now + TOKEN_TTL_SECONDS }));
...
export function verifyAccessToken(token: string) {
  ...
  const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as { sub?: string; companyId?: string; exp?: number };
  if (!parsed.sub || !parsed.companyId || !parsed.exp || parsed.exp <= Math.floor(Date.now() / 1000)) return null;
  return parsed;
}
